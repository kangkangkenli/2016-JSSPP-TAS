...
for number = min(head(j), tail(j)): tail(j)
    ...
    if S(x,y,z,1) == 0 && mod(number,line) == 1
        ...
        % here need to restrict the search space to 
        % min(cur_num + job_size - 1,total_bin_size), the search cannot
        % exceed the size of the bin, overflow of array will occur
        
        for num = cur_num : min(cur_num + req_size - 1,tail(j))
            
             % for the first job_size - 1 nodes, they cannot be
             % on the last node of a plane, because this is
             % concave shape. If there is a turn, then the previous
             % nodes all do not count
